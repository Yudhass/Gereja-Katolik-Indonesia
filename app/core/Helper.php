...
// Start All Helper Functions Using Database Model ================================================================================
...
if (!function_exists('getUser')) {
    function getUser($id =  null)
    {
        ...
        if($id){
            $data = $modelUser->find((int)$id);
            return $data ? $data->nama : 'Unknown';
        } else {
            return 'Unknown';
        }
    }
...
if (!function_exists('CekAkses')) {
    function CekAkses($roleId = null, $permission = null)
    {
        if (!$roleId) {
            return false;
        }

        require_once dirname(__FILE__) . '/../models/ModelHakAkses.php';
        $modelHakAkses = new ModelHakAkses();

        $data = $modelHakAkses->where('role', (int)$roleId)->first();
        
        return $data && isset($data->$permission) && ($data->$permission == 1 || $data->$permission == '1');
    }
}


if (!function_exists('getDataDokter')) {
    function getDataDokter($id_dokter = null)
    {
        $db = DatabaseManager::connection('tera');
        $sql = "SELECT pid, name_real from person where pid<0 and is_doctor is true and ihs_nakes is not null and pid = :id_dokter LIMIT 1";

        $result = $db->query($sql, array('id_dokter' => $id_dokter));
        $dataDokter = $db->fetchAll($result);
        $dataDokter = count($dataDokter) > 0 ? $dataDokter[0]->name_real : 'Unknown';
        return $dataDokter;
    }
}

if (!function_exists('getDataLinen')) {
    require_once dirname(__FILE__) . '/../models/ModelLinen.php';
    require_once dirname(__FILE__) . '/../models/ModelJenis.php';
    require_once dirname(__FILE__) . '/../models/ModelKategori.php';
    require_once dirname(__FILE__) . '/../models/ModelWarna.php';
    require_once dirname(__FILE__) . '/../models/ModelUkuran.php';
    function getDataLinen($rfid = null)
    {
        $modelLinen = new ModelLinen();
        $modelJenis = new ModelJenis();
        $modelKategori = new ModelKategori();
        $modelWarna = new ModelWarna();
        $modelUkuran = new ModelUkuran();
        $dataLinen = $modelLinen->where('rfid', $rfid)->first();
        $dataLinen->jenis_detail = $modelJenis->find($dataLinen->id_jenis);
        $dataLinen->kategori_detail = $modelKategori->find($dataLinen->jenis_detail->id_kategori);
        $dataLinen->warna_detail = $modelWarna->find($dataLinen->jenis_detail->id_warna);
        $dataLinen->ukuran_detail = $modelUkuran->find($dataLinen->jenis_detail->id_ukuran);

        return $dataLinen;
    }
}
// Akhir All Helper Functions Using Database Model ================================================================================
